...
class WarehouseEnvironment:
    ...
    def reset(self) -> EnvironmentState:
        ...
        # 从case数据初始化作业
        if 'jobs' in self.case:
            for job_data in self.case['jobs']:
                operations = [
                    Operation(
                        operation_id=op_data['operation_id'],
                        available_machines=op_data['available_machines'],
                ...
                job = Job(
                    ...
                    arrival_time=job_data.get('arrival_time', 0)
                )
        ...
        for m in range(self.config.problem.num_machines):
            capabilities = []
            if 'machine_capabilities' in self.case:
                capabilities = self.case['machine_capabilities'].get(m, [])
    ...
    def step(self, actions):
        ...
        next_state = EnvironmentState.from_dict(self._get_state())
    ...
    def _is_valid_batch_assignment(self, job, batch):
        """检查批次分配是否可行"""
        if job.distributor_id != batch['distributor']:
            return False
    ...
    def _calculate_tardiness(self):
        ...
        for job in self.jobs:
            if job.complete_time is not None:
                ...
                total_tardiness += tardiness * job.priority
    ...
    def _calculate_batch_utilization(self):
        ...
        return completed / len(self.batches)
