...
In the production stage, the shop floor consists of $m \in \{10, 20, 30\}$ heterogeneous machines. The system is initially loaded with $n_{\text{init}}=20$ jobs, and an additional $n_{\text{add}} \in \{30, 50, 70\}$ jobs arrive dynamically over time. Job arrivals follow a Poisson process with an average rate $\lambda = 0.5$. Each job comprises $n_i \sim \mathcal{U}[3,5]$ sequential operations, with processing times $t_{ij} \sim \mathcal{U}[5,15]$ time units. Machine eligibility for each operation is randomly assigned. Each job $j$ also has an associated goods quantity $q_j \sim \mathcal{U}[1,5]$ to be delivered upon completion.

In the distribution stage, the system includes $d \in \{3, 5, 7\}$ distributors. Each distributor has $w_d \sim \mathcal{U}[2,3]$ due time windows, with a delivery coverage ratio $\rho_d \sim \mathcal{U}[0.2,1]$ representing the proportion of completed goods required to meet delivery commitments. Unfulfilled deliveries incur a penalty weight $w_d \sim \mathcal{U}[0.5,1]$. These settings collectively capture the diversity of delivery urgency, resource constraints, and personalization demands in realistic production–distribution systems.

To simulate realistic delivery requirements in community group buying scenarios, we implement a distributor-based differentiated strategy for generating delivery time windows. The distributors are categorized into three types based on their service characteristics: \textit{express distributors} (ID $\bmod 3 = 0$) with time preference weights $[3.0, 2.0, 1.0]$ favoring early delivery windows, \textit{standard distributors} (ID $\bmod 3 = 1$) with uniform weights maintaining balanced time distribution, and \textit{economic distributors} (ID $\bmod 3 = 2$) with weights $[1.0, 2.0, 3.0]$ biased toward later delivery windows for cost optimization. For each distributor $d$, the number of delivery requirements $n_{req}$ is randomly sampled from $\mathcal{U}[2, 3]$, and the corresponding due times $\delta_d$ are generated using cumulative weight distribution: $\delta_{d,i} = t_{min} + \frac{\sum_{j=1}^{i} w_j}{\sum_{k=1}^{n_{req}} w_k} \times (t_{max} - t_{min})$, where $w_j$ represents the type-specific weights, ensuring that different distributor types exhibit distinct temporal delivery patterns that reflect real-world service differentiation.
...
Number of initial jobs ($n_{\text{init}}$) & 20 \\
Number of dynamically arrived jobs ($n_{\text{add}}$) & \{30, 50, 70\} \\
Number of operations per job ($n_i$) & $\mathcal{U}[3, 5]$ \\
Operation processing time ($t_{ij}$) & $\mathcal{U}[5, 15]$ \\
Goods quantity of jobs ($q_j$) & $\mathcal{U}[1, 5]$ \\
Number of distributors ($d$) & \{3, 5, 7\} \\
Number of due time windows per distributor ($w_d$) & $\mathcal{U}[2, 3]$ \\
Due time windoww of distributors ($\delta_d$) & $\mathcal{U}[10, 20]$ \\
...
\begin{table}[htbp]
\centering
\caption{Total tardiness comparison of heuristic baselines and the proposed HDRL method.}
\label{tab:heuristic_tardiness}
\begin{tabular}{ccccccccccc}
\toprule
\textbf{Instance ID} & \textbf{\#Jobs} & \textbf{\#Machines} & \textbf{\#Distributors} & \textbf{EDD} & \textbf{SPT} & \textbf{LPT} & \textbf{CR} & \textbf{Local Search} & \textbf{GA} & \textbf{Proposed} \\
\midrule
