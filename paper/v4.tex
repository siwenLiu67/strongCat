...
\begin{algorithmic}[1]
\State Initialize meta-policy network \( \pi_\theta \), action counts \( N(a) \), total steps \( T \leftarrow 0 \)
...
The experiments are conducted using a simulated production environment that mimics real-world job shop scenarios. We consider a range of problem instances with varying characteristics, including the number of jobs, machines, and resource constraints. The performance metrics used for evaluation include makespan, total tardiness, and resource utilization.
