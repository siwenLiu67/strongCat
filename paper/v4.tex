...
Mean inter-arrival time ($E_{\text{arr}}$) & \{20, 40, 60\} \\
Number of operations per job ($n_i$) & U[3, 6] \\
Operation processing time ($t_{ij}$) & U[10, 50] \\
Job type ($u_i$) & B(1, 0) \\
Expiration time urgency factor ($\mathit{ETUF}_i$) & \{1.5 ($u_i = 1$), 1 ($u_i = 0$)\} \\
Due date of job $j$ ($\delta_j$) & $C_j^{\text{est}} + \Delta_j$, where $\Delta_j \sim \mathcal{U}(20, 40)$ \\
Minimum batch size ($b_{\min}$) & \{2, 3\} \\
Maximum batch waiting time ($T_{\text{wait}}^{\max}$) & \{20, 30\} \\
\bottomrule
\end{tabular}
\end{table}
...
To evaluate the effectiveness of the proposed method, four representative heuristic algorithms were selected as baseline methods. These algorithms are widely adopted in production scheduling and dispatching problems due to their interpretability, low computational cost, and robust performance in practical applications.

\textbf{(1) Priority Rule Algorithm (PR).}  
This method applies predefined dispatching rules (e.g., earliest due date, shortest processing time, or minimum slack time) to determine the sequencing of jobs. The rule is applied iteratively until all jobs are scheduled and dispatched.

\textbf{(2) Greedy Construction Algorithm (GC).}  
A constructive heuristic that schedules operations in a stepwise manner, always selecting the job–machine assignment with the most immediate benefit (e.g., minimum additional completion time or tardiness) until the solution is complete.

\textbf{(3) Local Search Algorithm (LS).}  
An improvement-based approach that starts from an initial feasible schedule and iteratively explores its neighborhood (e.g., job swaps, operation reassignments) to reduce the objective function. The search stops when no improving move can be found within the neighborhood.

\textbf{(4) Genetic Algorithm (GA).}  
A population-based metaheuristic that evolves a set of candidate schedules using genetic operators, including selection, crossover, and mutation. The encoding scheme represents job–machine assignments, and the fitness function is based on the multi-objective performance criteria.

\begin{table}[htbp]
\centering
\caption{Performance comparison of heuristic baseline algorithms.}
\label{tab:heuristic_baselines}
\begin{tabular}{lcccccccc}
\toprule
\textbf{Algorithm} & \textbf{Instance ID} & \textbf{\#Jobs} & \textbf{\#Machines} & \textbf{Total Tardiness} & \textbf{Weighted Shortage} & \textbf{Objective Sum} & \textbf{Makespan} & \textbf{On-time Rate} \\
\midrule
PR  & Small-01 & 8  & 4  & 149.0 & 0.0 & 149.0 & 61.0 & 0.375 \\
GC  & Small-01 & 8  & 4  & 140.5 & 0.0 & 140.5 & 59.0 & 0.417 \\
LS  & Small-01 & 8  & 4  & 133.2 & 0.0 & 133.2 & 58.0 & 0.458 \\
GA  & Small-01 & 8  & 4  & 128.7 & 0.0 & 128.7 & 57.0 & 0.500 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering  
\caption{Performance of heuristic baselines: (a) Objective Sum; (b) Service-level related metrics (On-time Rate, Request Coverage).}
\label{fig:heuristic_baselines}
\end{figure}

As shown in Table~\ref{tab:heuristic_baselines}, metaheuristic methods (LS and GA) generally outperform simple constructive heuristics (PR and GC) in terms of the total tardiness and the composite objective value. The GA achieves the best average performance by effectively balancing job completion timeliness and delivery coverage. Figure~\ref{fig:heuristic_baselines} further illustrates that improvement-based and population-based heuristics yield higher service-level metrics, indicating their potential advantages in scenarios with tight delivery constraints.
