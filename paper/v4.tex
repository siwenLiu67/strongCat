...
In the distribution stage, the system involves $d \in \{10, 15, 20\}$ distributors. Moreover, service heterogeneity is introduced: express distributors prioritize earlier due times, standard distributors adopt balanced settings, and economic distributors allow for more relaxed deadlines to minimize cost. For each distributor $d$, the number of delivery requirements $n_{req}$ is sampled from $\mathcal{U}[2,3]$.
For each distributor $d$, a set of $n_{\rm req}$ due times $\{\delta_{d,1},\ldots,\delta_{d,n_{\rm req}}\}$ is generated within a feasible interval $[t_{\min}^{(d)},\,t_{\max}^{(d)}]$. Here, $t_{\min}^{(d)}$ and $t_{\max}^{(d)}$ are determined as the sum of the minimum and maximum attainable processing times of all operations in the assigned job set, evaluated across their eligible machines in the production stage. To incorporate service heterogeneity, distributors are categorized into three types according to $d \bmod 3$: express distributors with early delivery preference, standard distributors with balanced delivery preference, and economic distributors with late delivery preference. For each distributor, a raw weight vector $w^{(d)}=(w^{(d)}_1,\ldots,w^{(d)}_{n_{\rm req}})$ is constructed according to type-specific preferences, then normalized into probabilities $\tilde w^{(d)}_i=w^{(d)}_i/\sum_{k=1}^{n_{\rm req}} w^{(d)}_k$ and accumulated into a distribution $\hat w^{(d)}_i=\sum_{k=1}^i \tilde w^{(d)}_k$. To ensure robustness against scheduling fluctuations, the upper bound is extended to $t_{\max}^{(d)}+buffer$. The due times are subsequently defined at the cumulative quantiles of the interval as
\[
\delta_{d,i} \;=\; t_{\min}^{(d)} \;+\; \hat w^{(d)}_i \Bigl( (t_{\max}^{(d)}+buffer) - t_{\min}^{(d)} \Bigr), \qquad i=1,\ldots,n_{\rm req}.
\]
This procedure guarantees that all jobs under the same distributor share a coherent delivery horizon while accommodating heterogeneous service patterns across distributor types.
