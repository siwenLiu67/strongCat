...
def _extract_unified_metrics(stats: Dict, env: Any, config: Any, total_time: float) -> Dict:
    ...
    metrics.update(_extract_metrics(stats, env, config, total_time))
    
    return metrics

def _extract_metrics(stats: Dict, env: Any, config: Any, total_time: float) -> Dict:
    """提取强化学习算法指标"""
    ...
    metrics = {
        ...
        'algorithm_type': stats.get('algorithm_type', 'Unknown'),
    }
