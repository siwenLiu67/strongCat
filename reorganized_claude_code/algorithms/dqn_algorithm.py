...
class DQNAlgorithm:
    ...
    def solve(self, instance_data: Dict, algorithm_params: Dict = None) -> Dict:
        ...
        print(f"开始DQN训练 {episodes} episodes...")
        ...
        for episode in range(episodes):
            ...
            stats['epsilon'].append(self.agent.epsilon)
    ...
    def _action_to_dict(self, action: int) -> Dict:
        ...
        Returns:
            环境动作字典
        """
        # 简化版本的动作映射，实际实现需要根据具体动作空间设计
        action_dict = {
            'action_type': 'schedule',
            'job_id': action % 100,  # 假设最多100个作业
            'machine_id': (action // 100) % 10,  # 假设最多10台机器
            'operation_id': (action // 1000) % 10  # 假设最多10个工序
        }
        return action_dict
