...
class WarehouseEnvironment:
    ...
    def available_actions(self) -> List[Dict]:
        ...
        # 添加等待动作
        actions.append({'wait': True})
        
        # 添加调度动作
        for job in self.available_jobs:
            if job.status == 'waiting' and job.current_operation < len(job.operations):
                op = job.operations[job.current_operation]
                for machine_id in op.available_machine_ids:
                    machine = next((m for m in self.machines if m.machine_id == machine_id and m.status == 'waiting'), None)
                    if machine:
                        actions.append({
                            'schedule': {job.job_id: machine_id}
                        })
                        
        # 添加配送动作
        completed_jobs = [j for j in self.completed_jobs if j.status == 'completed']
        for distributor in self.distributors:
            if completed_jobs:
                actions.append({
                    'dispatch': {distributor.distributor_id: [j.job_id for j in completed_jobs]}
                })
