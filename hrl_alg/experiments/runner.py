...
from results.algorithm_results_saver import save_algorithm_results_csv, set_random_seed
...
class UniversalAlgorithmRunner:
    ...
    def get_algorithm_function(self, algo_name: str) -> Callable:
        ...
        try:
            ...
            # 统一使用run_experiment作为主函数名
            func_name = 'run_experiment'
            print(f"尝试获取函数: {func_name} from {module_name}")
            
            # 尝试获取run_experiment函数
            if hasattr(module, func_name):
                return getattr(module, func_name)
            
            # 如果run_experiment不存在，尝试备选函数名
            alternative_names = [
                'main',
                f'run_{algo_name.lower()}_experiment',
