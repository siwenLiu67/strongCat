...
class A2CAlgorithm(BaseAlgorithm):
    ...
    def solve(self, production_data: Dict, transport_data: Dict, orders_data: Dict, num_episodes: int = 5) -> Dict:
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                # n-step更新
                if step % self.n_steps == 0 or done:
                    self._update_a2c(states, actions, rewards, log_probs, values, done)
                    # 清空轨迹
                    states, actions, rewards, log_probs, values = [], [], [], [], []
    ...
    def _update_a2c(self, states, actions, rewards, log_probs, values, done):
        """A2C算法更新"""
        if len(states) == 0 or self.optimizer is None:
            return
        
        # 计算n-step回报
        returns = self._compute_n_step_returns(rewards, values[-1] if not done else 0, self.gamma, self.n_steps)
    ...
    def _compute_n_step_returns(self, rewards, last_value, gamma, n_steps):
        """计算n-step回报"""
        returns = []
        R = last_value
