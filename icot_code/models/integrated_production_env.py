...
class IntegratedFJSPEnv(gym.Env):
    """
    ...
    def __init__(self, production_data, orders_data, transport_data, normalize_obs: bool = False, seed: Optional[int] = None):
        ...
        # 时间： [current_time, T_internal - current_time, slack_time]
        state_size = (self.num_machines * 3) + (self.num_jobs * 5) 
    ...
    def _get_state(self):
        ...
        for job_id in self.jobs_data:
            ...
            slack = due - self.current_time - rpt  # slack time
            job_states.extend([cur, fin, rpt, due, slack])


        state = np.array(machine_states + job_states, dtype=np.float32)
    ...
    def step(self, action):
        ...
        # === 调用运输计划检查可行性 ===
        feasible = self._check_transport_feasibility()
        if not feasible:
            reward -= 1000  # 强惩罚，不可行
    ...
    def _check_transport_feasibility(self) -> bool:
        """检查运输可行性：所有订单必须在deadline前交付"""
        for order_id, order in self.orders_data.items():
            deadline = order.get('due')
            market = order.get('market')

            if deadline is None or market is None or market not in self.transport_data:
                continue # 如果订单信息不完整，则跳过检查

            # 找到到该市场所有运输方式中的最短运输时间
            min_lead_time = float('inf')
            market_transport_options = self.transport_data[market]
            for mode_options in market_transport_options.values():
                for provider_details in mode_options.values():
                    lead_time = provider_details.get('lead_time_hours')
                    if lead_time is not None:
                        min_lead_time = min(min_lead_time, lead_time)

            if min_lead_time == float('inf'):
                return False # 没有可用的运输方式

            finish_time = self.current_time + min_lead_time
            if finish_time > deadline:
                return False # 即使最快的运输方式也无法按时送达
        return True
    ...
    def _calculate_reward(self):
        """
        计算奖励函数。
        - 中间奖励：每完成一步给予一个小的正奖励，鼓励探索。
        - 最终奖励：当所有工序完成时，根据总成本计算一个大的奖励或惩罚。
        """
        # 1. 中间奖励 (Dense Reward)
        # 每成功调度一步（即一个工序被分配），给予一个小的正奖励以鼓励其继续进行。
        reward = 1.0
    ...
    def plan_transportation(self, production_finish_time: float) -> tuple[float, dict]:
        """
        一个简化的运输规划函数，用于在环境内部评估成本。
        返回 (总运输成本, 运输方案)。如果不可行，成本为无穷大。
        """
        total_transport_cost = 0
        transport_plan = {}

        for order_id, order_details in self.orders_data.items():
            market = order_details['market']
            due_date = order_details['due']
            
            # 查找从工厂到该市场的可用运输方式和时间
            # 这里简化为只考虑一种运输模式
            transport_options = self.transport_data.get(market, [])
            if not transport_options:
                return float('inf'), {} # 没有到该市场的运输方式

            # 选择最快的运输方式来检查可行性
            best_option = min(transport_options, key=lambda x: x['time'])
            transport_time = best_option['time']
            
            # 检查是否能在截止日期前送达
            if production_finish_time + transport_time > due_date:
                return float('inf'), {} # 不可行

            # 如果可行，累加成本
            total_transport_cost += best_option['cost']
            transport_plan[order_id] = {
                'mode': best_option['type'],
                'start_time': production_finish_time,
                'arrival_time': production_finish_time + transport_time
            }
