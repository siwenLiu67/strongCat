...
from icot_code.comparison_algorithms.metaheuristic_algorithms import GeneticAlgorithm, SimulatedAnnealing, ParticleSwarmOptimization
...
def run_experiment_on_instance(instance_path: str, T_internal_values: List[int]) -> List[Dict]:
    ...
    algorithms = [
        ...
        EDDRule(),
        CompositeRule(),
        GeneticAlgorithm(population_size=30, generations=50),  # 较小规模以节省时间
        SimulatedAnnealing(initial_temperature=500.0, max_iterations=200),  # 较小规模以节省时间
        ParticleSwarmOptimization(swarm_size=20, max_iterations=50)  # 较小规模以节省时间
    ]
...
def main():
    ...
    if all_results:
        ...
        output_file = "results/metaheuristic_comparison_results.csv"
        ...
        metaheuristic_algorithms = ['GeneticAlgorithm', 'SimulatedAnnealing', 'ParticleSwarmOptimization']
        
        heuristic_df = df[df['algorithm'].isin(heuristic_algorithms)]
        metaheuristic_df = df[df['algorithm'].isin(metaheuristic_algorithms)]
        ...
        print("\n元启发式算法统计:")
        print(metaheuristic_df.groupby('algorithm').agg({
            'total_cost': ['mean', 'std'],
            'computation_time': 'mean'
        }).round(2))
